...
import { ChatMessage, PlumeResponse, Tone, Length, AppState, Idea, ViewType, User, Fidelity, PhotoCatalystResult, DigitalMemory, GuestContext } from './types';
...
const App: React.FC = () => {
    ...
    // Backup input text to prevent loss
    useEffect(() => {
        if (input) localStorage.setItem('plume_input_backup', input);
    }, [input]);

    // Restore input text on mount
    useEffect(() => {
        const saved = localStorage.getItem('plume_input_backup');
    ...
    // --- Realtime Notifications for Guest Contributions ---
    useEffect(() => {
        if (!session?.user) return;

        const channel = supabase
            .channel('guest-contributions')
            .on(
                'postgres_changes',
                { event: 'INSERT', schema: 'public', table: 'guest_contributions' },
                async (payload) => {
                    const newContribution = payload.new as { chapter_id: string };
                    // Verify if this contribution is for one of the user's chapters
                    const { data } = await supabase
                        .from('chapters')
                        .select('user_id, title')
                        .eq('id', newContribution.chapter_id)
                        .single();

                    if (data && data.user_id === session.user.id) {
                        showToast(`🎉 Nouvelle contribution pour "${data.title}" !`, 'success');
    ...
    const loadUserData = async (authUser: any) => {
        ...
        try {
            ...
            if (profileData && !profileError) {
                ...
                setShowProfileModal(true);
                logger.warn("Profile not found or incomplete, showing modal.");
            }
            ...
            } catch (e) { logger.warn("Erreur chargement brouillon", e); }
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            if (draftContent || workspaceId) {
                ...
                } catch (err) { logger.error("Auto-save failed:", err); showToast("Erreur auto-sauvegarde du brouillon.", "error"); }
            }
        }, 2000);
    ...
    const triggerSend = useCallback(async (text: string, imageUrl?: string) => {
        ...
        try {
            ...
            logger.error("Erreur Gemini lors de l'envoi du message:", error);
    ...
    const handleAddToDraft = async (text: string, messageId: string) => {
        ...
        // Block assistant messages from being added to draft
        if (message?.role === 'assistant') {
            showToast("Les messages de Plume ne peuvent pas être ajoutés à l'ébauche.", 'error');
            return;
        }
    ...
    const handleConfirmRegeneration = async () => {
        ...
        try {
            ...
            logger.error("Erreur durant la régénération du message:", error);
    ...
    const handleNewSequence = async () => {
        ...
        // Check if there are unsaved user messages
        const userMessages = state.messages.filter(m => m.role === 'user' && m.id !== 'welcome');

        if (userMessages.length > 0) {
            // Ask user if they want to save before clearing
            const shouldSave = window.confirm(
                "Vous avez une conversation en cours. Voulez-vous sauvegarder ce souvenir avant de passer à un nouveau sujet ?\n\n" +
                "✅ Oui : Le souvenir sera sauvegardé en brouillon\n" +
                "❌ Non : La conversation sera effacée"
            );

            if (shouldSave) {
                // Extract conversation text for draft
                ...
                if (conversationText.trim()) {
                    try {
                        // Generate metadata from conversation
                        showToast("Sauvegarde du souvenir en cours...", 'info');
                        const { title, dates, characters, tags } = await generateTitleAndMetadata(conversationText);

                        // Save as draft chapter
                        const { error } = await supabase.from('chapters').insert({
                            user_id: session.user.id,
                            title: title || "Souvenir en cours",
                        ...
                        // Save entities
                        if (dates) for (const date of dates) await saveEntityToDB('date', date, session.user.id);
                        if (characters) for (const char of characters) await saveEntityToDB('person', char, session.user.id);
                        if (tags) for (const tag of tags) await saveEntityToDB('theme', tag, session.user.id);

                        showToast(`Souvenir "${title}" sauvegardé en brouillon !`, 'success');
                    } catch (error) {
                        logger.error("Error saving draft:", error);
                        showToast("Erreur lors de la sauvegarde. Le souvenir sera perdu.", 'error');

                        // Ask again if they want to continue despite the error
                        const continueAnyway = window.confirm("La sauvegarde a échoué. Voulez-vous quand même effacer la conversation ?");
                        if (!continueAnyway) {
                            return; // Abort new sequence
                        }
                    }
                }
            }
        ...
        try {
            ...
            showToast("Erreur lors de la création de la séquence", 'error');
...
Je souhaite raconter ce souvenir. Aide-moi à le développer en me posant une question inspirante basée sur les suggestions.`;
    ...
    const handleSouvenirShare = (souvenir: any) => {
        // Prepare the context for the guest
        const context: GuestContext = {
            souvenirId: souvenir.id,
            authorName: userProfile?.firstName || 'L\'auteur',
            memoryTitle: souvenir.title || 'Un souvenir partagé',
            memoryContext: {
                location: 'Lieu non spécifié', // In a real app, extract from metadata
                date: souvenir.dates?.[0] || 'Date inconnue',
                tags: souvenir.tags || []
            },
            authorQuestion: "J'aimerais avoir ta version de cette histoire. Qu'est-ce qui t'a le plus marqué ce jour-là ?" // Default question, could be customizable
        };

        setGuestContext(context);
        setCurrentView('guest_prototype');
        showToast("Mode invité activé pour ce souvenir", 'success');
    };
        ...
        <div className="flex h-screen bg-paper text-ink-900 font-sans">
            ...
            <div className="fixed top-0 w-full h-20 bg-white/95 backdrop-blur-sm text-ink-900 z-40 flex items-center justify-between px-8 shadow-sm border-b border-ink-100">
                ...
                <nav className="hidden md:flex items-center gap-2 bg-ink-50 p-1.5 rounded-xl border border-ink-100">
                    <button onClick={() => setCurrentView('studio')} className={`nav-btn ${currentView === 'studio' ? 'active' : ''}`}><IconLayout className="w-5 h-5" />Atelier des Souvenirs</button>
                    ...
                    <button onClick={() => setCurrentView('boutique')} className={`nav-btn ${currentView === 'boutique' ? 'active' : ''}`}><IconSearch className="w-5 h-5" />Boutique des Souvenirs</button>
            ...
            <div className="flex flex-1 pt-20 h-screen w-full overflow-y-auto">{/* Ajout de h-screen pour fixer la hauteur */}
                {currentView === 'studio' && (
                    <>
                        {!focusMode && (
                            ...
                            {!focusMode && (
                                <div className="w-full bg-white border-t border-ink-100 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.03)] z-20 mb-16 md:mb-0">
                                    <div className="max-w-5xl mx-auto space-y-3">
                                        {/* Settings moved to input bar */}

                                        <div className="relative group bg-white border-2 border-ink-100 rounded-2xl focus-within:border-accent focus-within:ring-4 focus-within:ring-accent/10 transition-all shadow-sm hover:shadow-md">
                                            {showSettings && (
                                                <div className="absolute bottom-full left-0 mb-4 z-30 w-full md:w-[500px] max-h-[60vh] overflow-y-auto p-4 bg-white rounded-2xl border border-ink-200 shadow-2xl animate-fade-in custom-scrollbar">
                                                    <div className="flex justify-between items-center mb-4 border-b border-ink-100 pb-2 sticky top-0 bg-white z-10">
                                                        <h3 className="font-serif font-bold text-lg text-ink-800 flex items-center gap-2"><IconFeather className="w-5 h-5 text-accent" />Studio de Style</h3>
                                                        <button onClick={() => setShowSettings(false)} className="text-ink-400 hover:text-ink-600 hover:bg-ink-100 p-1 rounded-full transition-colors"><IconX className="w-5 h-5" /></button>
                                                    </div>
                                                    <StyleSelector
                                                        tone={tone}
                                                        length={length}
                                            ...
                                            {/* Toolbar Area - Always below text */}
                                            <div className="px-2 pb-2 md:px-4 md:pb-3 flex justify-between items-center bg-transparent rounded-b-2xl">
                                                {/* Left Actions */}
                                                <div className="flex gap-1 md:gap-2">
                                                    <button onClick={() => setShowSettings(!showSettings)} disabled={isLoading || isRecording} className={`input-btn ${showSettings ? 'text-accent bg-accent/10' : ''}`} title="Style & Ton"><IconFeather className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                    <button onClick={() => setShowTimeContext(!showTimeContext)} disabled={isLoading || isRecording} className={`input-btn ${timeContext ? 'text-accent bg-accent/10' : ''}`} title="Contexte Temporel"><IconClock className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                    <button onClick={() => setShowPhotoCatalyst(true)} disabled={isLoading || isRecording} className="input-btn hover:bg-accent/10 hover:text-accent" title="Catalyseur Photo"><IconCamera className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                </div>
                                                ...
                                                <div className="flex gap-2 items-center">
                                                    {regenerationInfo ? (
                                                        <>
                                                            <button onClick={handleCancelRegeneration} className="p-2 rounded-xl bg-ink-100 text-ink-600 hover:bg-ink-200 transition-colors" title="Annuler"><IconX className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                            <button onClick={handleConfirmRegeneration} className="p-2 rounded-xl bg-accent text-white shadow-lg hover:bg-accent-light transition-colors flex items-center gap-2 px-4 font-medium" title="Confirmer"><IconRefresh className="w-5 h-5 md:w-6 md:h-6" /><span className="hidden md:inline">Régénérer</span></button>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <button onClick={handleNewSequence} disabled={isLoading || isRecording} className="input-btn" title="Nouveau sujet"><IconScissors className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                            <button onClick={handleSynthesis} disabled={isLoading || isRecording} className="input-btn" title="Générer le Récit - Transforme votre conversation en récit structuré"><IconMagic className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                            {!isRecording ? (
                                                                <>
                                                                    <button onClick={handleStartRecording} disabled={isLoading} className="input-btn hover:bg-red-50 hover:text-red-500" title="Dictée vocale"><IconMicrophone className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                                    <button onClick={handleSendMessage} disabled={!input.trim() || isLoading} className={`p-2 md:p-3 rounded-xl transition-all ${input.trim() && !isLoading ? 'bg-accent text-white shadow-lg hover:bg-accent-light hover:shadow-xl transform hover:-translate-y-0.5' : 'bg-ink-100 text-ink-300 cursor-not-allowed'}`} title="Envoyer"><IconSend className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                                </>
                                                            ) : (
                                                                <button onClick={handleStopRecording} className="p-2 md:p-3 rounded-xl bg-red-500 text-white shadow-lg hover:bg-red-600 animate-pulse" title="Arrêter l'enregistrement"><IconStopCircle className="w-5 h-5 md:w-6 md:h-6" /></button>
                                                            )}
                                                        </>
                                                    )}
                                                </div>
                ...
                {currentView === 'boutique' && session?.user && (<div className="w-full h-full pb-16 md:pb-0"><BoutiqueSouvenirs userId={session.user.id} onSouvenirSelect={handleSouvenirSelect} onSouvenirShare={handleSouvenirShare} /></div>)}
                ...
                {currentView === 'guest_prototype' && (
                    <div className="w-full h-full overflow-y-auto">
                        <GuestLandingPage
                            guestContext={guestContext || {} as any}
